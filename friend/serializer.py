...
class FriendshipRequestSerializer(ModelSerializer):
    ...
    def validate(self, data):  # 존재하는 유저 및 이미 친구인지 검사
        ...
        if query.exists():
            raise serializers.ValidationError(
                {"message": "이미 친구요청이 존재합니다."}, code=status.HTTP_409_CONFLICT
            )

        query = Friend.objects.filter(user=data["sender"], friend=data["receiver"])
        if query.exists():
            raise serializers.ValidationError(
                {"message": "이미 친구입니다."}, code=status.HTTP_409_CONFLICT
            )
    ...
    def create(self, validated_data):
        ...
        friendship = FriendshipRequest.objects.create(sender=sender, receiver=receiver)
        friendship.save()
        return friendship
